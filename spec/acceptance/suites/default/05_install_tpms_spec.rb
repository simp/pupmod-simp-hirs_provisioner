...
describe 'install tpm_simulators' do
  ...
  def config_abrmd_for_tpm2sim_on(hirs_host)
    tpm2_abrmd_version = on(hirs_host, 'tpm2-abrmd --version').stdout.split(%r{\s+}).last
    tcti_info = if tpm2_abrmd_version.split('.').first.to_i > 1
                  '--tcti=/usr/lib64/libtss2-tcti-mssim.so.0'
                else
                  '-t socket'
                end
